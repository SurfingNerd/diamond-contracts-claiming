...
const prefix = "I accept the DMDv4 Whitepaper terms and claim my airdrop to: ";

async function fillMainnet() {
    ...
    const snapshot = JSON.parse(fs.readFileSync('snapshots/snapshot-mainnet_1851895.json', 'utf8')) as BalanceSnapshot;
    ...
    console.log(`npx hardhat verify --network mainnet ${claimContractAddress} ${claimBeneficorAddress} ${beneficorDAOAddress} ${prefixHex} ${dillute1} ${dillute2} ${dillute3}` );
